/* =========================================================================
   app.js — نسخة واحدة (Supabase فقط) + نقل كل inline scripts من HTML
   ========================================================================= */
...
(() => {
  ...
  if (!createClient) {
    console.error("Supabase SDK not loaded. تأكد إن supabase-js اتحمّل قبل app.js");
    return;
  }
  ...
  // ───────────────────────────────────────────────────────────────────────
  const PAIR_CONFIG = {
    "EUR/USD": { basePrice: 1.0854, seed: 11001, digits: 5, vb: 8e-4, tb: 5e-5 },
    "AUD/CAD": { basePrice: 0.8952, seed: 22002, digits: 5, vb: 7e-4, tb: 4e-5 },
    "AUD/CHF": { basePrice: 0.5748, seed: 33003, digits: 5, vb: 6e-4, tb: 4e-5 },
    "USD/CHF": { basePrice: 0.9052, seed: 44004, digits: 5, vb: 7e-4, tb: 4e-5 },
    "EUR/RUB": { basePrice: 95.5, seed: 55005, digits: 3, vb: 0.08, tb: 5e-3 },
    "AED/CNY": { basePrice: 1.953, seed: 66006, digits: 5, vb: 8e-4, tb: 5e-5 },
    "BHD/CNY": { basePrice: 18.54, seed: 77007, digits: 4, vb: 0.015, tb: 1e-3 },
    ...
    "QAR/CNY": { basePrice: 1.972, seed: 10010, digits: 5, vb: 8e-4, tb: 5e-5 },
    "SYP/TRY": { basePrice: 0.00095, seed: 21021, digits: 6, vb: 5e-6, tb: 3e-7 },
    "EGP/USD": { basePrice: 0.0205, seed: 32032, digits: 6, vb: 1e-4, tb: 6e-6 },
    "USD/INR": { basePrice: 83.52, seed: 43043, digits: 4, vb: 0.07, tb: 4e-3 },
  };

  // ───────────────────────────────────────────────────────────────────────
  // 2) Helpers: Trades camelCase <-> snake_case
  // ───────────────────────────────────────────────────────────────────────
  ...
  function showChartPnlOverlay(pnlStr, amount) {
    ...
    if (!document.getElementById("_chartPnlCSS")) {
      ...
      s.id = "_chartPnlCSS";
      s.textContent =
        "#chartPnlOverlay{position:absolute;top:10px;left:10px;z-index:80;padding:6px 10px;border-radius:8px;background:rgba(0,0,0,.55);border:1px solid rgba(255,255,255,.10);color:#fff;font-size:11px;font-family:monospace;letter-spacing:.3px;opacity:0;transform:translateY(-6px);transition:opacity .25s ease,transform .25s ease;pointer-events:none;white-space:nowrap;}#chartPnlOverlay.show{opacity:1;transform:translateY(0);}";
  ...
  window.updateBalanceDisplay = refreshUI;
  ...
  window.deductBalance = async (amt) => {
    ...
    try {
      await _supabase.from("users").update({ [field]: val }).eq("id", _uid);
    } catch (err) {
      console.warn("deductBalance:", err);
    }
  };
  ...
  window.setUserAvatar = async (url) => {
    ...
    if (!_uid) return;
    try {
      await _supabase.from("users").update({ avatar: url }).eq("id", _uid);
    } catch (err) {
      console.warn("setUserAvatar:", err);
    }
  };

  // ───────────────────────────────────────────────────────────────────────
  // 5) Supabase: init user fields + realtime user listener
  // ───────────────────────────────────────────────────────────────────────
  async function _initUserFields(userId) {
    ...
    if (Object.keys(updates).length) await _supabase.from("users").update(updates).eq("id", userId);
  }
  ...
  // ───────────────────────────────────────────────────────────────────────
  // 6) Auth helpers (على window زي ما طلبت)
  // ───────────────────────────────────────────────────────────────────────
  window.authSignUp = async (email, password) => _supabase.auth.signUp({ email, password });
  window.authSignIn = async (email, password) => _supabase.auth.signInWithPassword({ email, password });
  window.authSignOut = async () => _supabase.auth.signOut();
  window.authSignInGoogle = async () =>
    _supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: window.location.origin } });
  ...
  async function openTrade(type) {
    ...
    try {
      const { data: row, error } = await _supabase.from("trades").insert(_tradeToDb(tradeData)).select().single();
      if (error) throw error;

      const trade = { ...tradeData, id: row.id };
      activeTrades.push(trade);

      if (window.chart) window.chart.addMarkerForTrade(type, trade);
      renderActiveTrades();
  ...
  async function closeTrade(trade) {
    ...
    try {
      await _supabase
        .from("trades")
        .update({ status: newStatus, close_price: closePrice, pnl: pnl, closed_at: new Date().toISOString() })
        .eq("id", trade.id);
    } catch (err) {
      console.warn("closeTrade update:", err);
    }
  ...
  async function loadActiveTrades() {
    if (!_uid) return;
    try {
      const { data: rows, error } = await _supabase.from("trades").select("*").eq("uid", _uid).eq("status", "active");
      if (error) throw error;
      ...
      console.warn("loadActiveTrades:", err);
    }
  }
  ...
  async function loadHistory() {
    ...
    try {
      ...
      const accFilter =
        (t) => (curAcc === "demo" ? !t.account || t.account === "demo" : t.account === curAcc);
      ...
      const netPnl =
        wonT.reduce((s, t) => s + (t.pnl || 0), 0) + lostT.reduce((s, t) => s + (t.pnl || 0), 0);
  ...
  function renderTradeRecord(t) {
    ...
    const close = t.closePrice ? t.closePrice.toFixed(5) : t.status === "active" ? "⏳" : "--";
  ...
  // ───────────────────────────────────────────────────────────────────────
  // 11) Chart Master Manager + Chart (كما كان — Supabase)
  // ───────────────────────────────────────────────────────────────────────
  class ChartMasterManager {
    ...
    async claimMaster(pair) {
      ...
      try {
        const { data } = await _supabase.rpc("claim_master", { p_key: key, p_session_id: this.sessionId });
        claimed = (data === true);
      } catch (err) {
        console.warn("claimMaster error:", err);
        claimed = true; // fallback
      }
    ...
    _startWatch(pair) {
      ...
      this._watchTimers[pair] = setInterval(async () => {
        ...
        try {
          const { data } = await _supabase.from("chart_control").select("*").eq("id", this._masterKey(pair)).maybeSingle();
          const now = Date.now();
          ...
          if (isDead) {
            ...
            if (became && window.chart && window.chart.currentPair === pair) window.chart._onBecameMaster(pair);
          }
        } catch (err) {}
      }, 7000);
    }
    ...
    broadcastLiveCandle(candle, pair) {
      ...
      _supabase.from("chart_control")
        .upsert({ id: this._liveKey(pair), candle: candle, pair: pair, updated_at: new Date().toISOString() }, { onConflict: "id" })
        .catch(() => {});
    }
  ...
  class AdvancedTradingChart {
    constructor() {
      ...
      if (!this.plot || !this.canvas) throw new Error("Chart DOM elements not found");
      ...
      this.priceRange = { min: this.basePrice * 0.99, max: this.basePrice * 1.01 };

      this.baseSpacing = 12;
      this.zoom = 1; this.targetZoom = 1;
      this.minZoom = 0.425; this.maxZoom = 2.25;
      this.zoomEase = 0.28;
      this.targetOffsetX = 0; this.offsetX = 0;
      ...
      this.smin = null; this.smax = null; this.sre = 0.088;
      this._fr = 0;
    ...
    _injectSkeletonStyles() {
      ...
      s.id = "_skelCSS";
      s.textContent =
        '#chartSkeleton{position:absolute;inset:0;z-index:9999;background:url("https://xdcbitr.com/h5/chart-loading-animation.gif") center center/100% 100% no-repeat;pointer-events:none;opacity:1;transition:opacity .25s ease;}#chartSkeleton.sk-hidden{opacity:0;}';
      document.head.appendChild(s);
    }
    _createSkeleton() {
    ...
    _buildCCPHtml(timeLeft) {
      ...
      ${rows.join("")}
      <div class="ccp-progress-bar"><div class="ccp-progress-fill" id="ccpProgressFill" style="width:100%"></div></div>`;
    }
    _syncCCPSelections() {
      const panel = document.getElementById("candleControlPanel");
      if (!panel) return;
    ...
    genCandle(t, o) {
      ...
      const r1 = this.rndG(s), r2 = this.rndG(s + 1), r3 = this.rndG(s + 2), r4 = this.rnd(s + 3),
        r5 = this.rnd(s + 4), r6 = this.rnd(s + 5);
      const v = vb * (0.7 + Math.abs(r1) * 0.8), tr = tb * r2 * 0.6, dir = r3 > 0 ? 1 : -1, tc = o + (dir * v + tr);
      const rg = v * (0.2 + r4 * 0.6), hm = rg * (0.3 + r5 * 0.7), lm = rg * (0.3 + (1 - r5) * 0.7);
      const c = +(tc + (r6 - 0.5) * v * 0.1).toFixed(this.digits), op = +o.toFixed(this.digits);
      return {
        open: op,
        close: c,
    ...
    async _batchSaveCandles(pairKey, candles) {
      ...
      const rows = candles.map(c => ({
        pair: pairKey, timestamp: c.timestamp, open: c.open, close: c.close, high: c.high, low: c.low
      }));
    ...
    _startViewerListener(pair) {
      ...
      const ch = _supabase
        ...
        .on("postgres_changes", { event: "*", schema: "public", table: "chart_control", filter: `id=eq.${liveKey}` }, payload => {
    ...
    snapToLive() { this.targetOffsetX = this.getMinOffset(); this.offsetX = this.targetOffsetX; this.velocity = 0; this.getMinMaxOffsetClamp(); }
    updatePan() {
    ...
    updateCandleTimer() {
      ...
      const n = Date.now(), e = n - this.t0, r = this.timeframe - e, s = Math.floor(r / 1000);
    ...
    addMarkerFromTrade(trade) {
      const candleTs =
        trade.candleTimestamp !== undefined && trade.candleTimestamp !== null
          ? trade.candleTimestamp
          : Math.floor((trade.startTime || Date.now()) / this.timeframe) * this.timeframe;

      const markerPrice =
        trade.markerPrice !== undefined && trade.markerPrice !== null ? trade.markerPrice : trade.entryPrice;

      this.markers.push({
        type: trade.type,
        ts: trade.startTime,
        price: markerPrice,
        candleIndex: trade.candleIndex || 0,
    ...
    drawMarker(m) {
      ...
      if (actualIdx === null || actualIdx === undefined) actualIdx = m.candleIndex ?? this.candles.length;
    ...
    updateCurrentCandle() {
      ...
      const dir = (r - 0.5) * vb * 0.5;
      const tgt = this.currentCandle.close + dir;
      const ms = vb * 0.18;
    ...
    initEvents() {
      ...
      this.canvas.addEventListener("wheel", e => {
        ...
        const nz = Math.max(this.minZoom, Math.min(this.maxZoom, oz * (e.deltaY > 0 ? 1 / 1.1 : 1.1)));
      ...
      this.canvas.addEventListener("touchmove", e => {
        ...
        if (this.pinch && e.touches.length === 2) {
          ...
          if (this.p0 > 0) {
            ...
            if (Math.abs(nz - oz) > 1e-6) {
              const idx = this.xToIndex(this.pMidX);
              this.targetZoom = nz;
              this.zoom = nz;
              const nx = this.pMidX - idx * this.getSpacing();
              this.targetOffsetX = nx;
              this.offsetX = nx;
              this.getMinMaxOffsetClamp();
              this.updateTimeLabels();
  ...
  // ───────────────────────────────────────────────────────────────────────
  // 12) UI: Account menu handlers + buy/sell protection + selector handlers
  // ───────────────────────────────────────────────────────────────────────
  function initUIWiring() {
    ...
    document.addEventListener("click", (e) => {
      if (!e.target.closest("#balanceBox")) {
        _accMenu?.classList.remove("show");
        _balBox?.classList.remove("open");
      }
    });
    ...
    if (_switchAccBtn) {
      _switchAccBtn.addEventListener("click", (e) => {
        ...
        refreshUI();
        showInfoToast(window._curAcc === "demo" ? "✅ تم التبديل إلى الحساب التجريبي" : "✅ تم التبديل إلى الحساب الحقيقي");
      });
    }
    ...
    compensationList?.addEventListener("click", (e) => {
      if (e.target.classList.contains("dropdown-item")) {
        ...
        timeDisplay.textContent = savedTimeValue;
        if (window.chart) window.chart.selectedTime = parseInt(e.target.getAttribute("data-sec"));
        timeDropdown.classList.remove("show");
      }
    });
  ...
  // ───────────────────────────────────────────────────────────────────────
  // 14) Pair Panel (كان inline — اتنقل هنا)
  // ───────────────────────────────────────────────────────────────────────
  function initPairPanel() {
    ...
    const dP = [
      { pair:"EUR/USD", flags:["EUR","USD"], payout:.92 },
      { pair:"AUD/CAD", flags:["AUD","CAD"], payout:.88 },
      { pair:"AUD/CHF", flags:["AUD","CHF"], payout:.92 },
      { pair:"USD/CHF", flags:["USD","CHF"], payout:.89 },
      { pair:"EUR/RUB", flags:["EUR","RUB"], payout:.77 },
      { pair:"AED/CNY", flags:["AED","CNY"], payout:.91 },
      { pair:"BHD/CNY", flags:["BHD","CNY"], payout:.86 },
      { pair:"KES/USD", flags:["KES","USD"], payout:.84 },
      { pair:"LBP/USD", flags:["LBP","USD"], payout:.79 },
      { pair:"QAR/CNY", flags:["QAR","CNY"], payout:.83 },
      { pair:"SYP/TRY", flags:["SYP","TRY"], payout:.87 },
      { pair:"EGP/USD", flags:["EGP","USD"], payout:.78 },
    ...
    function upPairHud(p) {
      ...
      const pf = document.getElementById("pairFlags");
      const pt = document.getElementById("pairHudTxt");
    ...
    function rPP() {
      ...
      const s = [...dP].sort((a, b) => {
        const fa = favs.has(a.pair) ? 1 : 0;
        const fb = favs.has(b.pair) ? 1 : 0;
        return fa !== fb ? fb - fa : a.pair.localeCompare(b.pair);
      });

      pl.innerHTML = s.map(x => `
        <button class="pit" data-p="${x.pair}">
          <span class="pitL">
            <span class="star" data-s="1">${favs.has(x.pair) ? "★" : "☆"}</span>
            <span class="fg">${x.flags.map(f => `<img src="${mkF(f)}" alt="${f}">`).join("")}</span>
            <span class="nm">${x.pair}<span class="bad">OTC</span></span>
          </span>
          <span style="font-size:11px;font-weight:900;color:#00dd00">+${fP(x.payout)}</span>
        </button>
      `).join("");
    }

    const pH = document.getElementById("pairHud");
    const pP = document.getElementById("pairPanel");
    const pC = document.getElementById("pairClose");
    const pL = document.getElementById("pairList");
    ...
    if (pL) pL.onclick = async e => {
      ...
      upPairHud(p);
      if (window.chart?.switchPair) await window.chart.switchPair(p);
    };

    document.addEventListener("click", e => {
      if (pP && pP.classList.contains("on") && !e.target.closest("#pairPanel") && !e.target.closest("#pairHud")) {
        pP.classList.remove("on");
      }
    });
  ...
  // ───────────────────────────────────────────────────────────────────────
  // 15) Rank Panel (كان inline — اتنقل هنا)
  // ───────────────────────────────────────────────────────────────────────
  function initRankPanel() {
    ...
    const pf = n => { n = +n || 0; return (n >= 0 ? "+$" : "-$") + Math.abs(n).toLocaleString("en-US", { minimumFractionDigits:2, maximumFractionDigits:2 }); };
    const rnd = a => a[Math.random() * a.length | 0];

    const mkTrader = () => {
      const f = ["أحمد","محمد","عمر","يوسف","خالد","سارة","ريم","هند","نورة","ليلى","فيصل","مازن","عبدالله","سلمان"];
      const l = ["العمري","الشريف","المطيري","القحطاني","الزهراني","الدوسري","العتيبي","الغامدي","المنصوري","البلوشي","الرشيدي"];
      return {
        name: `${rnd(f)} ${rnd(l)}`,
        accountId: String(1e8 + (Math.random() * 9e8 | 0)),
        trades: 60 + (Math.random() * 400 | 0),
        rate: 40 + (Math.random() * 55 | 0),
        profit: Math.round(((Math.random() * 90000) - 5000) * 100) / 100,
      };
    };
    ...
    function renderRank() {
      $("rankCount").textContent = traders.length;
      const top = Math.max(0, ...traders.map(t => +t.profit || 0));
      const avg = traders.length ? Math.round(traders.reduce((s, t) => s + (+t.rate || 0), 0) / traders.length) : 0;

      $("rankTopProfit").textContent = money(top);
      $("rankAvgRate").textContent = avg + "%";
      $("rankUpdated").textContent = new Date().toLocaleTimeString("en-GB");

      $("rankContent").innerHTML = traders
        .sort((a, b) => b.profit - a.profit)
        .map(a => `
          <div class="trade-record">
            <div class="tr-top">
              <div class="tr-badge">
                <span class="tr-type buy" style="border-color:rgba(255,215,0,.35);color:#ffd700;background:rgba(255,215,0,.12)">ID ${esc(a.accountId || "—")}</span>
                <span class="tr-status active">Live</span>
              </div>
              <div class="tr-pnl win">${pf(a.profit || 0)}</div>
            </div>
            <div class="tr-details">
              <div class="tr-detail"><div class="tr-detail-label">الاسم</div><div class="tr-detail-value">${esc(a.name || "—")}</div></div>
              <div class="tr-detail"><div class="tr-detail-label">Trades</div><div class="tr-detail-value">${+a.trades || 0}</div></div>
              <div class="tr-detail"><div class="tr-detail-label">Win%</div><div class="tr-detail-value">${+a.rate || 0}%</div></div>
            </div>
          </div>
        `).join("") || '<div class="empty-history">لا يوجد بيانات.</div>';
    }

    window.openRank = () => {
      const p = $("rankPanel");
      if (p) p.classList.add("show");
      if (!traders.length) for (let i = 0; i < 5; i++) traders.push(mkTrader());
      renderRank();
    };

    $("rankCloseBtn")?.addEventListener("click", () => $("rankPanel")?.classList.remove("show"));
  }

  // ───────────────────────────────────────────────────────────────────────
  // 16) Deposit Modal (كان inline — اتنقل هنا)
  // ───────────────────────────────────────────────────────────────────────
  function initDeposit() {
    ...
    const $ = id => document.getElementById(id);
    const SC = [ $("dS1"), $("dS2"), $("dS3") ];

    const st = { amt: 4300, cur: "TRY", bon: false };
    const stk = [0];
    const amts = [42350,105800,4300,10550,1300,2200,450,900];

    const sym = c => (c === "USD" ? "$" : "₺");
    const fmt = n => String(Math.max(0, Math.floor(n || 0))).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const base = () => Math.max(0, Math.floor(st.amt || 0));
    const bon = () => st.bon ? Math.floor(base() * BR) : 0;
    const tot = () => base() + bon();
    const mny = n => sym(st.cur) + fmt(n);

    let wt = null;

    function show(k) {
      SC.forEach((x, i) => x?.classList.toggle("on", i === k));
      if (stk[stk.length - 1] !== k) stk.push(k);

      $("dBack").style.opacity = stk.length > 1 ? 1 : 0.45;
      sync();
    ...
    function stopWait() { if (wt) { clearInterval(wt); wt = null; } }

    function sync() {
      $("dCur").value = st.cur;
      $("dAmtIn").value = base();
      $("dPayBtn").textContent = "ادفع " + fmt(base()) + " " + sym(st.cur);

      $("dConfBal").textContent = mny(base());
      $("dConfCur").textContent = st.cur;

      $("dAddrTxt").textContent = ADDR;

      $("dBaseLine").textContent = mny(base());
      $("dBonusLine").textContent = st.bon ? ("+" + mny(bon()) + " (" + Math.round(BR * 100) + "%)") : "—";
      $("dTotalLine").textContent = mny(tot());

      $("dS3amt").textContent = mny(tot());

      $("dQRimg").src =
        "https://api.qrserver.com/v1/create-qr-code/?size=190x190&data=" +
        encodeURIComponent(JSON.stringify({ network:"USDT_TRC20", address:ADDR, currency: st.cur, base: base(), bonus: bon(), total: tot() }));

      $("dS3send").textContent = "USDT_TRC20";
    }

    function buildAmts() {
      $("dAmtGrid").innerHTML = amts.map(v => `<div class="damt ${v === st.amt ? "on" : ""}" data-v="${v}">${sym(st.cur)}${fmt(v)}</div>`).join("");
      $("dAmtGrid").onclick = e => {
        const b = e.target.closest(".damt");
        if (!b) return;
        st.amt = +b.dataset.v;
        [...$("dAmtGrid").children].forEach(x => x.classList.toggle("on", +x.dataset.v === st.amt));
        sync();
      };
    }

    function repaint() {
      [...$("dAmtGrid").children].forEach(x => {
        const v = +x.dataset.v;
        x.textContent = sym(st.cur) + fmt(v);
      });
    }

    $("dBack").onclick = () => { if (stk.length > 1) { stk.pop(); show(stk.pop()); } };
    $("dClose").onclick = () => closeDeposit();
    $("dPayBtn").onclick = () => { if (!$("dAgree").checked) { alert("يرجى الموافقة على الشروط أولاً"); return; } show(1); };
    $("dPayNow").onclick = () => show(2);

    $("dCur").onchange = () => { st.cur = $("dCur").value; repaint(); sync(); };
    $("dAmtIn").oninput = () => {
      const v = +$("dAmtIn").value.replace(/[^\d]/g, "");
      st.amt = v || 0;
      [...$("dAmtGrid").children].forEach(x => x.classList.remove("on"));
      sync();
    };
    ...
    $("dCpy").onclick = async () => {
      try {
        ...
        $("dCpy").textContent = "✓";
        setTimeout(() => ($("dCpy").textContent = "⎘"), 900);
      } catch (e) {
        prompt("انسخ العنوان:", ADDR);
      }
    };

    $("dBonusToggle").onclick = () => {
      const b = $("dBonusBox");
      b.style.display = b.style.display === "block" ? "none" : "block";
    };

    $("dBonusApply").onclick = () => {
      const c = ($("dBonusIn").value || "").trim();
      if (!c) {
        $("dBonusState").innerHTML = '<div class="dBad">✖ اكتب رمز البونص</div>';
        st.bon = false;
        sync();
        return;
      }
      if (c === BC) {
        st.bon = true;
        $("dBonusState").innerHTML = '<div class="dOk">✔ تم تفعيل بونص ' + Math.round(BR * 100) + "%</div>";
      } else {
        st.bon = false;
        $("dBonusState").innerHTML = '<div class="dBad">✖ رمز غير صحيح</div>';
      }
      sync();
    };

    window.openDeposit = () => {
      $("depModal").style.transform = "translateY(0)";
      buildAmts();
      sync();
      stk.length = 0;
      stk.push(0);
    ...
    window.closeDeposit = () => {
      stopWait();
      $("depModal").style.transform = "translateY(100%)";
    };
  }

  // ───────────────────────────────────────────────────────────────────────
  // 17) Notifications (كان inline — اتنقل هنا)
  // ───────────────────────────────────────────────────────────────────────
  function initNotifications() {
    ...
    const data = [
      { t: "تنبيه", m: "تم تسجيل دخول جديد." },
      { t: "معلومة", m: "تابع السوق قبل فتح صفقات." },
      { t: "أمان", m: "فعّل 2FA لحماية حسابك." }
    ];

    window.openNotifications = () => {
      const p = $("notifPanel"), c = $("notifContent");
      if (!p || !c) return;
      p.classList.add("show");
      c.innerHTML = '<div class="loading-text">جاري تحميل الإشعارات...</div>';
        ...
        c.innerHTML = data.length
          ? data.map(x => `
              <div class="trade-record">
                <div class="tr-top">
                  <div class="tr-badge">
                    <span class="tr-type buy" style="border-color:rgba(255,215,0,.35);color:#ffd700;background:rgba(255,215,0,.12)">${esc(x.t)}</span>
                    <span class="tr-status active">New</span>
                  </div>
                  <div class="tr-pnl active-pnl" style="font-size:11px">الآن</div>
                </div>
                <div style="color:rgba(255,215,0,.75);font-weight:900;font-size:11px;line-height:1.6">${esc(x.m)}</div>
              </div>
            `).join("")
          : '<div class="empty-history">لا يوجد إشعارات.</div>';
      }, 350);
    };
    ...
    $("notifBtn")?.addEventListener("click", async () => {
      if (window.openNotifications) window.openNotifications();
        ...
        $("notifContent").innerHTML = '<div class="empty-history">لا يوجد إشعارات.</div>';
      }
    });
  }

  // ───────────────────────────────────────────────────────────────────────
  // 18) Minor guards/stubs (عشان مفيش profile/settings panels هنا)
  // ───────────────────────────────────────────────────────────────────────
  window.openProfile = () => showInfoToast("ℹ️ Profile مش مفعّل في النسخة دي", "error", 1600);
  window.openSettings = () => showInfoToast("ℹ️ Settings مش مفعّل في النسخة دي", "error", 1600);
  ...
  function initLongPressBuyCCP() {
    ...
    buyBtn.addEventListener("touchstart", _start, { passive: true });
  ...
  function initNavHeightVar() {
    const setNavH = () => {
      const h = document.querySelector(".bottom-nav")?.offsetHeight || 56;
  ...
  // ───────────────────────────────────────────────────────────────────────
  // 20) Auth state (المشكلة الأساسية كانت sb/_supabase — اتحلت هنا)
  // ───────────────────────────────────────────────────────────────────────
  _supabase.auth.onAuthStateChange(async (_event, session) => {
    ...
    if (user) {
      ...
      await _supabase.from("users").upsert({ id: _uid, email: user.email || "" }, { onConflict: "id" });
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      await initChartSystem();
      loadActiveTrades();
    } catch (e) {
      console.error("Bootstrap error:", e);
      showInfoToast("⚠️ حصل خطأ في تهيئة التطبيق — راجع Console", "error", 4000);
    }
  });
