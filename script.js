/* ═══════════════════════════════════════════════════════════════
   SUPABASE TRADING APP  –  استبدال كامل لـ Firebase بـ Supabase
   ═══════════════════════════════════════════════════════════════ */

// ── إعداد Supabase ──────────────────────────────────────────────
const _SUPA_URL = 'https://jgcbhbqofimmgwzzoia.supabase.co';
const _SUPA_KEY = 'sb_publishable_mkNhancggR91jzTuzFryJQ_mQgHS0Qm';
// _sb هو الـ client الرئيسي (يُستخدَم بدلاً من db + auth القديمَين)
const _sb = (window.supabase || supabase).createClient(_SUPA_URL, _SUPA_KEY);

/* ── إعدادات أزواج التداول (بدون تغيير) ─────────────────────── */
const PAIR_CONFIG={
...
let _uid = null, _userChannel = null;

const _fmtBal = v => '$' + Number(v).toLocaleString('en', {minimumFractionDigits:2, maximumFractionDigits:2});

/* ── دوال مساعدة للتجارة (Mapper) ────────────────────────────── */
function _mapTradeFromDB(r) {
  return {
    id: r.id, uid: r.uid, type: r.type, pair: r.pair,
    entryPrice: r.entry_price, amount: r.amount, duration: r.duration,
    startTime: r.start_time, endTime: r.end_time, status: r.status,
    closePrice: r.close_price, pnl: r.pnl,
    markerPrice: r.marker_price,
...
/* ── تهيئة حقول المستخدم في Supabase ─────────────────────────── */
async function _initUserFields(userId) {
  const { data } = await _sb.from('users').select('*').eq('id', userId).maybeSingle();
  const up = { id: userId };
  if (!data) {
    up.demo_balance = 10000;
    up.real_balance = 0;
    up.avatar = '';
    up.email = '';
  } else {
    if (data.demo_balance == null) up.demo_balance = 10000;
    if (data.real_balance == null) up.real_balance = 0;
    if (data.avatar    == null) up.avatar = '';
    if (data.email     == null) up.email = '';
  }
  if (Object.keys(up).length > 1) {
    await _sb.from('users').upsert(up, { onConflict: 'id' });
  }
}

/* ── الاستماع لتغييرات بيانات المستخدم (Realtime) ──────────────── */
async function _listenUser(userId) {
  // إلغاء الاشتراك القديم
  if (_userChannel) { await _sb.removeChannel(_userChannel); _userChannel = null; }

  // جلب البيانات الأولية
  const { data } = await _sb.from('users').select('*').eq('id', userId).maybeSingle();
  if (data) {
    if (typeof data.demo_balance === 'number') window._demoBalance = data.demo_balance;
    if (typeof data.real_balance === 'number') window._realBalance = data.real_balance;
    refreshUI();
    setAvatar(data.avatar || '');
  }

  // Realtime: الاستماع للتحديثات
  _userChannel = _sb
    .channel('user-watch-' + userId)
    .on('postgres_changes', {
      event: 'UPDATE',
      schema: 'public',
      table: 'users',
...
_sb.auth.onAuthStateChange(async (event, session) => {
  ...
  if (user) {
    _uid = user.id;
    // حفظ البريد الإلكتروني في Supabase
    await _sb.from('users').upsert({ id: _uid, email: user.email || '' }, { onConflict: 'id' });
    await _initUserFields(_uid);
    await _listenUser(_uid);
    // الصورة الشخصية من مزوّد الهوية
    ...
    _uid = null;
    if (_userChannel) { _sb.removeChannel(_userChannel); _userChannel = null; }
    activeTrades = [];
    if (window.chart) window.chart.clearAllTradeMarkers();
    window._demoBalance = 10000;
    window._realBalance = 0;
...
if(_refillBtn){
  _refillBtn.addEventListener('click', async e => {
    ...
    refreshUI();
    try {
      await _sb.from('users').update({ demo_balance: 10000 }).eq('id', _uid);
      showInfoToast('✅ تم تعبئة الحساب التجريبي بـ $10,000');
    } catch(e){ console.warn('refill:', e); }
  });
}

/* ── حماية زرَّي الشراء والبيع ──────────────────────────────── */
document.addEventListener('click', e => {
...
window.deductBalance = async amt => {
  ...
  try { await _sb.from('users').update({ [field]: val }).eq('id', _uid); }
...
window.setUserAvatar = async url => {
  setAvatar(url);
  if(!_uid) return;
  try { await _sb.from('users').update({ avatar: url }).eq('id', _uid); }
...
async function openTrade(type){
  ...
  const tradeRow = {
    uid: _uid, type, pair: pairName,
    entry_price: entryPrice, amount, duration,
    start_time: startTime, end_time: endTime,
    status: 'active', close_price: null, pnl: null,
  ...
  try {
    const { data, error } = await _sb.from('trades').insert(tradeRow).select().single();
    if(error) throw error;

    const trade = {
      id: data.id, uid: _uid, type, pair: pairName,
      entryPrice, amount, duration, startTime, endTime,
      status: 'active', closePrice: null, pnl: null,
      markerPrice, candleTimestamp, candleIndex,
      account: window._curAcc
    };
...
async function loadActiveTrades(){
  if(!_uid) return;
  try {
    const { data, error } = await _sb.from('trades').select('*').eq('uid', _uid).eq('status','active');
    if(error) throw error;
    const now = Date.now();
    for(const row of data){
      const trade = _mapTradeFromDB(row);
...
async function loadHistory(){
  ...
  try {
    const { data: allRows, error } = await _sb
      .from('trades').select('*')
      .eq('uid', _uid)
    ...
    const allSnap = allRows.map(_mapTradeFromDB);
...
/* ══════════════════════════════════════════════════════════════
   ChartMasterManager  –  إدارة Master/Viewer عبر Supabase
   ══════════════════════════════════════════════════════════════ */
class ChartMasterManager {
  ...
  async _registerSession(){
    try {
      await _sb.from('chart_sessions').upsert({
        session_id: this.sessionId,
        last_seen: new Date().toISOString(),
        active: true
      }, { onConflict: 'session_id' });
    } catch(e){ console.warn('_registerSession:', e); }

    setInterval(async () => {
      try {
        await _sb.from('chart_sessions')
  ...
  // استخدام دالة PostgreSQL للمطالبة بـ Master بشكل ذري
  async claimMaster(pair){
    let claimed = false;
    try {
      const { data, error } = await _sb.rpc('claim_chart_master', {
        p_pair:       pair,
        p_session_id: this.sessionId,
        p_now:        Date.now()
      });
      if(!error) claimed = data === true;
      else       claimed = true; // fallback: نفترض أننا Master
    } catch(e){
      console.warn('claimMaster rpc error:', e);
      claimed = true;
    }
  ...
  _masterControlId(pair){ return 'master_' + pair.replace('/','_'); }
  ...
  _startHeartbeat(pair){
    ...
    this._hbTimers[pair] = setInterval(async () => {
      ...
      try {
        await _sb.from('chart_control')
          .update({ last_heartbeat: new Date().toISOString(), is_active: true })
          .eq('id', this._masterControlId(pair));
      } catch(e){}
    }, 3000);
  }
  ...
  _startWatch(pair){
    ...
    this._watchTimers[pair] = setInterval(async () => {
      ...
      try {
        const { data } = await _sb.from('chart_control').select('*')
          .eq('id', this._masterControlId(pair)).maybeSingle();
        const now    = Date.now();
        const isDead = !data || !data.is_active ||
  ...
  async broadcastLiveCandle(candle, pair){
    if(!this.masterPairs.has(pair) || !candle) return;
    const now = Date.now();
    if((now - this._lastBroadcast) < 200) return;
    this._lastBroadcast = now;
    try {
      await _sb.from('live_candles').upsert({
        id:         'live_' + pair.replace('/','_'),
        candle:     candle,
        pair:       pair,
  ...
  subscribeToLiveCandle(pair, callback){
    if(this._liveChannel){ _sb.removeChannel(this._liveChannel); this._liveChannel = null; }
    this._liveChannel = _sb
      .channel('live-c-' + pair.replace('/','_'))
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'live_candles',
        filter: `id=eq.live_${pair.replace('/','_')}`
      }, payload => {
  ...
  async _cleanup(){
    for(const pair of this.masterPairs){
      try {
        await _sb.from('chart_control')
          .update({ is_active: false })
          .eq('id', this._masterControlId(pair));
...
/* ══════════════════════════════════════════════════════════════
   AdvancedTradingChart  –  الشارت الرئيسي مع Supabase
   ══════════════════════════════════════════════════════════════ */
class AdvancedTradingChart {
  ...
  genCandle(t,o){
    const vb=this.vb||8e-4, tb=this.tb||5e-5;
  ...
  /* ── Supabase: تحميل الشموع وحفظها ──────────────────────────── */
  // نستخدم جدول 'candles' مع حقل pair بدلاً من مجموعات منفصلة
  _pairKey(pair){ return (pair || this.currentPair); }
  ...
  async loadCandlesFromFirebase(pair=null, isMaster=true){
    ...
    try {
      ...
      if(!error && data && data.length >= 10){
        this.candles = data.reverse().map(c => ({
          open: +c.open, close: +c.close, high: +c.high, low: +c.low, timestamp: +c.timestamp
        }));
        this.currentPrice = this.candles[this.candles.length-1].close;
      } else {
        // توليد أولي
        const generated=[];
        let startP=this.basePrice, startT=Math.floor(Date.now()/this.timeframe)*this.timeframe - 30*this.timeframe;
        for(let i=0;i<30;i++){ const c=this.genCandle(startT,startP); generated.push(c); startP=c.close; startT+=this.timeframe; }
        if(isMaster) await this._batchSaveCandles(targetPair, generated);
        this.candles = generated;
        this.currentPrice = this.candles[this.candles.length-1].close;
      }
      await this._fillAndSaveGaps(targetPair, isMaster);
    } catch(e){
  ...
  async _fillAndSaveGaps(pair, isMaster){
    if(!this.candles.length) return;
    const last = this.candles[this.candles.length-1];
    const now  = Date.now();
    const cur0 = Math.floor(now/this.timeframe)*this.timeframe;
    let t=last.timestamp+this.timeframe, p=last.close;
    const gaps=[];
    while(t < cur0 && gaps.length < 300){ const c=this.genCandle(t,p); gaps.push(c); p=c.close; t+=this.timeframe; }
    if(gaps.length > 0){
      this.candles.push(...gaps);
      if(this.candles.length > this.maxCandles) this.candles=this.candles.slice(-this.maxCandles);
      this.currentPrice = this.candles[this.candles.length-1].close;
      if(isMaster) await this._batchSaveCandles(pair, gaps);
      console.log(`📊 [Gap-Fill] ${pair}: +${gaps.length} شمعة`);
    }
  }

  async _batchSaveCandles(pair, candles){
    if(!candles || !candles.length) return;
    const rows = candles.map(c => ({
      pair, timestamp: c.timestamp,
      open: c.open, close: c.close, high: c.high, low: c.low
    }));
    const chunks=[];
    for(let i=0;i<rows.length;i+=400) chunks.push(rows.slice(i,i+400));
    for(const chunk of chunks){
      try { await _sb.from('candles').upsert(chunk, { onConflict: 'pair,timestamp' }); }
  ...
  _initLocalFallback(){
    ...
    for(let i=0;i<30;i++){ const c=this.genCandle(t,p); this.candles.push(c); p=c.close; t+=this.timeframe; }
    this.currentPrice = this.candles[this.candles.length-1].close;
  }

  async saveCandleToFirebase(candle, pair=null){
    const targetPair = pair || this.currentPair;
    if(window.masterManager && !window.masterManager.isMaster(targetPair)) return;
    try {
      await _sb.from('candles').upsert({
        pair: targetPair,
  ...
  /* ── Realtime: مستمع المشاهد (Viewer) ───────────────────────── */
  _startViewerListener(pair){
    this._stopViewerListener();
    if(!window.masterManager) return;
    this._viewerChannel = _sb
      .channel('viewer-c-' + pair.replace('/','_'))
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'live_candles',
        filter: `id=eq.live_${pair.replace('/','_')}`
      }, payload => {
    ...
    this._viewerUnsub = () => {
      if(this._viewerChannel){ _sb.removeChannel(this._viewerChannel); this._viewerChannel=null; }
    };
  }
  ...
  _afterCandlesLoaded(){
    ...
    if(!this.currentCandle && this.candles.length){
      const last  = this.candles[this.candles.length-1];
      const t0now = Math.floor(Date.now()/this.timeframe)*this.timeframe;
  ...
  drawGrid(){
    ...
    for(let i=0;i<=count;i++){
      const p=min+i*step, y=this.priceToY(p); if(y<-5||y>this.h+5)continue;
      const mj=i%5===0; this.ctx.strokeStyle=mj?"rgba(255,215,0,.12)":"rgba(255,255,255,.05)"; this.ctx.lineWidth=mj?1:.8;
    ...
    for(let i=s;i<=e;i++){
      if(i%stepC!==0)continue; const x=this.indexToX(i); if(x<-5||x>this.w+5)continue;
      const mj=i%Math.round(stepC*5)===0; this.ctx.strokeStyle=mj?"rgba(255,215,0,.12)":"rgba(255,255,255,.05)"; this.ctx.lineWidth=mj?1:.8;
  ...
  updateTimeLabels(){
    ...
    for(let i=s;i<=e;i++){
      if(i%stepC!==0)continue; const x=this.indexToX(i); if(x<5||x>this.w-5)continue;
      const t=tS+i*this.timeframe, d=new Date(t), hh=String(d.getHours()).padStart(2,"0"), mm=String(d.getMinutes()).padStart(2,"0");
  ...
  addMarkerFromTrade(trade){
    ...
    this.markers.push({
      ...
      candleTimestamp: candleTs, tradeId: trade.id,
      endTime: trade.endTime, status: trade.status||'active',
  ...
  drawMarker(m){
    ...
    const y=this.priceToY(m.price), cw=this.getCandleWidth(), ib=m.type==="buy";
    const cl=ib?"#16a34a":"#ff3b3b", r=5.5;
    this.ctx.save(); this.ctx.shadowColor=cl; this.ctx.shadowBlur=9; this.ctx.fillStyle=cl;
    this.ctx.beginPath(); this.ctx.arc(x,y,r,0,2*Math.PI); this.ctx.fill(); this.ctx.shadowBlur=0;
    this.ctx.fillStyle="#fff"; this.ctx.save(); this.ctx.translate(x,y);
    if(!ib) this.ctx.rotate(Math.PI);
    this.ctx.beginPath(); this.ctx.moveTo(0,-2.8); this.ctx.lineTo(-2,.8); this.ctx.lineTo(-.65,.8);
    this.ctx.lineTo(-.65,2.8); this.ctx.lineTo(.65,2.8); this.ctx.lineTo(.65,.8); this.ctx.lineTo(2,.8);
    this.ctx.closePath(); this.ctx.fill(); this.ctx.restore();
    const lx=x+cw/2+3;
    let lw;
    if(m.duration){ const cForTrade=(m.duration*1000)/this.timeframe, pxForTrade=cForTrade*this.getSpacing(); lw=Math.min(Math.max(pxForTrade,55),Math.max(this.w-lx-18,55)); }
  ...
  draw(){
    ...
    for(let i=0;i<this.candles.length;i++){
      const x=this.indexToX(i); if(x<-60||x>this.w+60) continue; this.drawCandle(this.candles[i],x,0);
    }
  ...
  updatePriceRange(){
    ...
    const lo=sl.map(c=>c.low), hi=sl.map(c=>c.high), mn=Math.min(...lo), mx=Math.max(...hi);
  ...
  initEvents(){
    ...
    this.canvas.addEventListener("wheel",e=>{
      ...
      const sc=e.deltaY>0?1/1.1:1.1; this.applyZoomAround(x,y,sc);
    },{passive:false});
    ...
    this.canvas.addEventListener("touchstart",e=>{
      ...
      if(e.touches.length===1) md(e.touches[0].clientX-r.left,Date.now());
        ...
        this.drag=0; this.pinch=1; this.p0=db2(e.touches[0],e.touches[1]);
...
/* ══════════════════════════════════════════════════════════════
   ★  Long-Press على زر BUY (10 ثوانٍ) → لوحة التحكم بالشمعة
   ══════════════════════════════════════════════════════════════ */
(function(){
...
timeDisplay.addEventListener("blur",()=>{
  if(isEditingTime){
    ...
    if(v.length===0) v="0005"; v=v.padStart(4,"0");
    const h=v.slice(0,2), m=v.slice(2,4);
    savedTimeValue=`${h}:${m}`; timeDisplay.textContent=savedTimeValue; isEditingTime=false;
  }
});
