...
function getTradesRef(){if(!_uid)return null;return db.collection('users').doc(_uid).collection('trades');}
function refreshUI(){
  const ac=window._curAcc,cur=ac==='demo'?window._demoBalance:window._realBalance,el=id=>document.getElementById(id);
  ...
  if(el('topAccIcon'))el('topAccIcon').src=ac==='demo'?'https://cdn-icons-png.flaticon.com/128/1344/1344761.png':'https://flagcdn.com/w40/us.png';
...
function setAvatar(url){const l=document.getElementById('topLogoImg');if(!l)return;l.src=(url&&url.startsWith('http'))?url:'https://cdn-icons-png.flaticon.com/128/18921/18921105.png';}
function updateBalanceDisplay(){refreshUI();}
async function _initUserFields(userId){
  const ref=db.collection('users').doc(userId),snap=await ref.get(),d=snap.exists?snap.data():{},up={};
...
function _listenUser(userId){
  ...
  _userUnsub=db.collection('users').doc(userId).onSnapshot(snap=>{
    ...
    refreshUI();setAvatar(d.avatar||'');
  });
}
auth.onAuthStateChanged(async user=>{
  if(user){
    _uid=user.uid;
    await db.collection('users').doc(_uid).set({email:user.email||''},{merge:true});
    await _initUserFields(_uid);_listenUser(_uid);
    if(user.photoURL)setAvatar(user.photoURL);
    activeTrades=[];if(window.chart)window.chart.clearAllTradeMarkers();loadActiveTrades();
  }else{
    _uid=null;if(_userUnsub){_userUnsub();_userUnsub=null;}
    activeTrades=[];if(window.chart)window.chart.clearAllTradeMarkers();
    window._demoBalance=10000;window._realBalance=0;refreshUI();setAvatar('');
  }
});

const _balBox=document.getElementById('balanceBox'),_accMenu=document.getElementById('accMenu');
if(_balBox){_balBox.addEventListener('click',function(e){if(e.target.closest('.accMenu'))return;e.stopPropagation();_accMenu.classList.toggle('show');_balBox.classList.toggle('open');});}
document.addEventListener('click',e=>{if(!e.target.closest('#balanceBox')){if(_accMenu)_accMenu.classList.remove('show');if(_balBox)_balBox.classList.remove('open');}});
const _accItemDemo=document.getElementById('accItemDemo');
if(_accItemDemo){_accItemDemo.addEventListener('click',e=>{if(e.target.closest('#refillRow'))return;window._curAcc='demo';localStorage.setItem('_curAcc','demo');refreshUI();if(_accMenu)_accMenu.classList.remove('show');if(_balBox)_balBox.classList.remove('open');});}
const _accItemReal=document.getElementById('accItemReal');
if(_accItemReal){_accItemReal.addEventListener('click',()=>{window._curAcc='real';localStorage.setItem('_curAcc','real');refreshUI();if(_accMenu)_accMenu.classList.remove('show');if(_balBox)_balBox.classList.remove('open');});}
const _switchAccBtn=document.getElementById('switchAccBtn');
if(_switchAccBtn){_switchAccBtn.addEventListener('click',e=>{e.stopPropagation();window._curAcc=window._curAcc==='demo'?'real':'demo';localStorage.setItem('_curAcc',window._curAcc);refreshUI();showInfoToast(window._curAcc==='demo'?'✅ تم التبديل إلى الحساب التجريبي':'✅ تم التبديل إلى الحساب الحقيقي');});}
const _refillBtn=document.getElementById('refillBtn');
if(_refillBtn){_refillBtn.addEventListener('click',async e=>{e.stopPropagation();if(!_uid){showInfoToast('❌ لازم تسجّل دخول الأول','error');return;}window._demoBalance=10000;refreshUI();try{await db.collection('users').doc(_uid).update({demoBalance:10000});showInfoToast('✅ تم تعبئة الحساب التجريبي بـ $10,000');}catch(e){console.warn('refill:',e);}});}

document.addEventListener('click',e=>{
  const buyB=document.getElementById('buyBtn'),sellB=document.getElementById('sellBtn');
  if(e.target!==buyB&&e.target!==sellB)return;
  const bal=window.getCurrentBalance(),amt=parseFloat((document.getElementById('amountDisplay').value||'0').replace(/[^0-9.]/g,''))||0;
...
window.deductBalance=async amt=>{
  ...
  refreshUI();if(!_uid)return;
  const field=window._curAcc==='demo'?'demoBalance':'realBalance',val=window._curAcc==='demo'?window._demoBalance:window._realBalance;
  try{await db.collection('users').doc(_uid).update({[field]:val});}catch(e){console.warn('deductBalance:',e);}
};
window.addBalance=async amt=>{
  ...
  refreshUI();if(!_uid)return;
  const field=window._curAcc==='demo'?'demoBalance':'realBalance',val=window._curAcc==='demo'?window._demoBalance:window._realBalance;
  try{await db.collection('users').doc(_uid).update({[field]:val});}catch(e){console.warn('addBalance:',e);}
};
window.setUserAvatar=async url=>{setAvatar(url);if(!_uid)return;try{await db.collection('users').doc(_uid).update({avatar:url});}catch(e){console.warn('setUserAvatar:',e);}};
refreshUI();

let _infoTimer=null;
function showInfoToast(msg,type='info',dur=3000){const el=document.getElementById('infoToast');if(!el)return;el.textContent=msg;el.className='info-toast show '+type;if(_infoTimer)clearTimeout(_infoTimer);_infoTimer=setTimeout(()=>el.classList.remove('show'),dur);}

let _chartResTimer=null;
function showChartPnlOverlay(pnlStr,amount){
  const plot=document.getElementById('plot');if(!plot)return;
  if(!document.getElementById('_chartPnlCSS')){const s=document.createElement('style');s.id='_chartPnlCSS';s.textContent=`#chartPnlOverlay{position:absolute;top:10px;left:10px;z-index:80;padding:6px 10px;border-radius:8px;background:rgba(0,0,0,.55);border:1px solid rgba(255,255,255,.10);color:#fff;font-size:11px;font-family:monospace;letter-spacing:.3px;opacity:0;transform:translateY(-6px);transition:opacity .25s ease,transform .25s ease;pointer-events:none;white-space:nowrap;}#chartPnlOverlay.show{opacity:1;transform:translateY(0);}`;document.head.appendChild(s);}
  ...
  el.textContent=`${pnlStr}  |  رأس المال: $${amount.toFixed(2)}`;el.classList.add('show');
  if(_chartResTimer)clearTimeout(_chartResTimer);_chartResTimer=setTimeout(()=>el.classList.remove('show'),2500);
}

function fmtCountdown(sec){sec=Math.max(0,Math.floor(sec));if(sec>=3600){const h=Math.floor(sec/3600),m=Math.floor((sec%3600)/60),s=sec%60;return`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;}else if(sec>=60){const m=Math.floor(sec/60),s=sec%60;return`${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;}return String(sec).padStart(2,'0')+'s';}
function fmtDate(ts){if(!ts)return'--';const d=new Date(ts),months=['يناير','فبراير','مارس','أبريل','مايو','يونيو','يوليو','أغسطس','سبتمبر','أكتوبر','نوفمبر','ديسمبر'];return`${d.getDate()} ${months[d.getMonth()]} ${d.getFullYear()} • ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;}

let activeTrades=[];

async function openTrade(type){
  if(!_uid)return showInfoToast('❌ لازم تسجّل دخول عشان الصفقات تبقى خاصة بيك','error');
  const raw=document.getElementById('amountDisplay').value.replace(/[^0-9.]/g,''),amount=parseFloat(raw)||50,bal=window.getCurrentBalance();
  ...
  const entryPrice=window.chart.currentPrice,duration=window.chart.selectedTime,startTime=Date.now(),endTime=startTime+duration*1000;
  const pairName=(window.chart.currentPair||'EUR/USD')+' OTC';
  const cc=window.chart.currentCandle,candleTimestamp=cc?cc.timestamp:Math.floor(startTime/window.chart.timeframe)*window.chart.timeframe,candleIndex=window.chart.candles.length;
  let markerPrice=entryPrice;
  if(cc){const bt=Math.max(cc.open,cc.close),bb=Math.min(cc.open,cc.close);if(markerPrice>bt)markerPrice=bt;else if(markerPrice<bb)markerPrice=bb;}
  await window.deductBalance(amount);
  const tradeData={uid:_uid,type,pair:pairName,entryPrice,amount,duration,startTime,endTime,status:'active',closePrice:null,pnl:null,markerPrice,candleTimestamp,candleIndex,account:window._curAcc};
  try{
    const tradesRef=getTradesRef();if(!tradesRef)throw new Error('No tradesRef');
    const ref=await tradesRef.add({...tradeData,createdAt:firebase.firestore.FieldValue.serverTimestamp()});
    const trade={...tradeData,id:ref.id};activeTrades.push(trade);
    window.chart.addMarkerForTrade(type,trade);renderActiveTrades();
    const rem=Math.max(0,endTime-Date.now());setTimeout(()=>closeTrade(trade),rem);
    showInfoToast(`${type==='buy'?'↗ شراء':'↘ بيع'}  $${amount}  @  ${entryPrice.toFixed(5)}`,type,2500);
  }catch(e){await window.addBalance(amount);console.error('openTrade:',e);showInfoToast('❌ خطأ في فتح الصفقة','error');}
}

async function closeTrade(trade){
  activeTrades=activeTrades.filter(t=>t.id!==trade.id);
  const closePrice=window.chart?window.chart.currentPrice:trade.entryPrice,won=trade.type==='buy'?closePrice>trade.entryPrice:closePrice<trade.entryPrice;
  const profit=won?+(trade.amount*0.8).toFixed(2):0,pnl=won?profit:-trade.amount,newStatus=won?'won':'lost';
  if(won)await window.addBalance(trade.amount+profit);
  try{const tradesRef=getTradesRef();if(tradesRef)await tradesRef.doc(trade.id).update({status:newStatus,closePrice,pnl,closedAt:firebase.firestore.FieldValue.serverTimestamp()});}catch(e){console.warn('closeTrade update:',e);}
  ...
  const hp=document.getElementById('historyPanel');if(hp&&hp.classList.contains('show'))loadHistory();
}

async function loadActiveTrades(){
  if(!_uid)return;const tradesRef=getTradesRef();if(!tradesRef)return;
  try{
    const snap=await tradesRef.where('status','==','active').get(),now=Date.now();
...
function renderActiveTrades(){const panel=document.getElementById('activeTradesPanel');if(!panel)return;panel.innerHTML='';panel.style.display='none';}
setInterval(()=>{const now=Date.now();for(const t of activeTrades){const el=document.getElementById(`histTimer-${t.id}`);if(el)el.textContent='⏱ '+fmtCountdown(Math.max(0,(t.endTime-now)/1000));}},1000);

async function loadHistory(){
  const content=document.getElementById('historyContent');if(!content)return;
  if(!_uid){content.innerHTML='<div class="empty-history">لازم تسجّل دخول عشان تشوف سجلّك</div>';return;}
  content.innerHTML='<div class="loading-text">⏳ جاري التحميل...</div>';
  try{
    const tradesRef=getTradesRef(),snap=await tradesRef.orderBy('startTime','desc').limit(100).get();
    const curAcc=window._curAcc,allSnap=snap.docs.map(d=>({id:d.id,...d.data()}));
    const accFilter=t=>curAcc==='demo'?(!t.account||t.account==='demo'):t.account===curAcc;
    const accTrades=allSnap.filter(accFilter);let trades=[...accTrades];
    const activeTab=document.querySelector('#historyPanel .htab.active'),tab=activeTab?activeTab.getAttribute('data-tab'):'all';
    if(tab==='active')trades=trades.filter(t=>t.status==='active');
    else if(tab==='closed')trades=trades.filter(t=>t.status==='won'||t.status==='lost');
    const wonT=accTrades.filter(t=>t.status==='won'),lostT=accTrades.filter(t=>t.status==='lost'),netPnl=wonT.reduce((s,t)=>s+(t.pnl||0),0)+lostT.reduce((s,t)=>s+(t.pnl||0),0);
    ...
    if(se('statPnl')){se('statPnl').textContent=(netPnl>=0?'+':'')+`$${netPnl.toFixed(2)}`;se('statPnl').style.color=netPnl>=0?'#00cc00':'#ff5555';}
  ...
  }catch(e){console.error('loadHistory:',e);content.innerHTML='<div class="empty-history">⚠️ خطأ في تحميل السجل</div>';}
}

function renderTradeRecord(t){
  const tc=t.type==='buy'?'buy':'sell',dir=t.type==='buy'?'↗ شراء':'↘ بيع';
  let stLbl,stCls,pnlCls,pnlStr;
  if(t.status==='active'){const rem=Math.max(0,((t.endTime||Date.now())-Date.now())/1000);stLbl='● نشطة';stCls='active';pnlCls='active-pnl';pnlStr=`<span id="histTimer-${t.id}">⏱ ${fmtCountdown(rem)}</span>`;}
  else if(t.status==='won'){stLbl='✓ ربح';stCls='won';pnlCls='win';pnlStr=`+$${(t.pnl||0).toFixed(2)}`;}
  else{stLbl='✗ خسارة';stCls='lost';pnlCls='loss';pnlStr=`-$${(t.amount||0).toFixed(2)}`;}
  const dur=fmtCountdown(t.duration||0),date=fmtDate(t.startTime),close=t.closePrice?t.closePrice.toFixed(5):(t.status==='active'?'⏳':'--');
  return`<div class="trade-record"><div class="tr-top"><div class="tr-badge"><span class="tr-type ${tc}">${dir}</span><span class="tr-status ${stCls}">${stLbl}</span></div><span class="tr-pnl ${pnlCls}">${pnlStr}</span></div><div class="tr-details"><div class="tr-detail"><span class="tr-detail-label">المبلغ</span><span class="tr-detail-value">$${(t.amount||0).toFixed(2)}</span></div><div class="tr-detail"><span class="tr-detail-label">سعر الدخول</span><span class="tr-detail-value">${(t.entryPrice||0).toFixed(5)}</span></div><div class="tr-detail"><span class="tr-detail-label">سعر الإغلاق</span><span class="tr-detail-value">${close}</span></div><div class="tr-detail"><span class="tr-detail-label">الزوج</span><span class="tr-detail-value">${t.pair||'EUR/USD'}</span></div><div class="tr-detail"><span class="tr-detail-label">المدة</span><span class="tr-detail-value">${dur}</span></div><div class="tr-detail"><span class="tr-detail-label">الوقت</span><span class="tr-detail-value" style="font-size:8px">${date}</span></div></div></div>`;
}

function updateLiveTime(){const d=new Date,u=d.getTime()+d.getTimezoneOffset()*6e4,t=new Date(u+108e5),h=String(t.getHours()).padStart(2,"0"),m=String(t.getMinutes()).padStart(2,"0"),s=String(t.getSeconds()).padStart(2,"0"),el=document.getElementById("liveTime");if(el)el.textContent=`${h}:${m}:${s} UTC+3`;}
updateLiveTime();setInterval(updateLiveTime,1e3);

class ChartMasterManager{
  constructor(){this.sessionId=Date.now().toString(36)+Math.random().toString(36).substr(2,8);this.masterPairs=new Set();this._hbTimers={};this._watchTimers={};this._sessionRef=db.collection('chart_sessions').doc(this.sessionId);this._registerSession();window.addEventListener('beforeunload',()=>this._cleanup());}
  _registerSession(){this._sessionRef.set({sessionId:this.sessionId,lastSeen:firebase.firestore.FieldValue.serverTimestamp(),active:true}).catch(()=>{});setInterval(()=>{this._sessionRef.update({lastSeen:firebase.firestore.FieldValue.serverTimestamp()}).catch(()=>{});},5000);}
  _masterRef(pair){return db.collection('chart_control').doc('master_'+pair.replace('/','_'));}
  _candleRef(pair){return db.collection('chart_control').doc('live_'+pair.replace('/','_'));}
  async claimMaster(pair){
    const ref=this._masterRef(pair);let claimed=false;
    try{await db.runTransaction(async tx=>{const snap=await tx.get(ref),now=Date.now(),isDead=!snap.exists||!snap.data().isActive||(now-(snap.data().lastHeartbeat?snap.data().lastHeartbeat.toMillis():0))>12000;if(isDead){tx.set(ref,{sessionId:this.sessionId,lastHeartbeat:firebase.firestore.FieldValue.serverTimestamp(),isActive:true,claimedAt:now});claimed=true;}});}catch(e){console.warn('claimMaster error:',e);claimed=true;}
    if(claimed){this.masterPairs.add(pair);this._startHeartbeat(pair);this._stopWatch(pair);console.log(`🎯 [Master] ${this.sessionId.slice(-6)} → ${pair}`);}else{this._startWatch(pair);console.log(`👁️ [Viewer] ${this.sessionId.slice(-6)} → ${pair}`);}
    return claimed;
  }
  _startHeartbeat(pair){if(this._hbTimers[pair])clearInterval(this._hbTimers[pair]);this._hbTimers[pair]=setInterval(()=>{if(!this.masterPairs.has(pair)){clearInterval(this._hbTimers[pair]);delete this._hbTimers[pair];return;}this._masterRef(pair).update({lastHeartbeat:firebase.firestore.FieldValue.serverTimestamp(),isActive:true}).catch(()=>{});},3000);}
  _startWatch(pair){if(this._watchTimers[pair])return;this._watchTimers[pair]=setInterval(async()=>{if(this.masterPairs.has(pair)){this._stopWatch(pair);return;}try{const snap=await this._masterRef(pair).get(),now=Date.now(),isDead=!snap.exists||!snap.data().isActive||(now-(snap.data().lastHeartbeat?snap.data().lastHeartbeat.toMillis():0))>12000;if(isDead){const became=await this.claimMaster(pair);if(became&&window.chart&&window.chart.currentPair===pair)window.chart._onBecameMaster(pair);}}catch(e){}},7000);}
  _stopWatch(pair){if(this._watchTimers[pair]){clearInterval(this._watchTimers[pair]);delete this._watchTimers[pair];}}
  isMaster(pair){return this.masterPairs.has(pair);}
  broadcastLiveCandle(candle,pair){if(!this.masterPairs.has(pair)||!candle)return;const now=Date.now();if(this._lastBroadcast&&(now-this._lastBroadcast)<200)return;this._lastBroadcast=now;this._candleRef(pair).set({candle:{...candle},pair,updatedAt:firebase.firestore.FieldValue.serverTimestamp()}).catch(()=>{});}
  subscribeToLiveCandle(pair,callback){if(this._liveUnsub){this._liveUnsub();this._liveUnsub=null;}this._liveUnsub=this._candleRef(pair).onSnapshot(snap=>{if(!snap.exists)return;const data=snap.data();if(data&&data.pair===pair&&data.candle)callback(data.candle);});}
  unsubscribeLive(){if(this._liveUnsub){this._liveUnsub();this._liveUnsub=null;}}
  releaseMaster(pair){if(!this.masterPairs.has(pair))return;this.masterPairs.delete(pair);if(this._hbTimers[pair]){clearInterval(this._hbTimers[pair]);delete this._hbTimers[pair];}this._masterRef(pair).update({isActive:false}).catch(()=>{});this._startWatch(pair);}
  _cleanup(){this.masterPairs.forEach(pair=>{try{this._masterRef(pair).update({isActive:false});}catch(e){}if(this._hbTimers[pair])clearInterval(this._hbTimers[pair]);});try{this._sessionRef.update({active:false});}catch(e){}}
}

class AdvancedTradingChart{
  constructor(){
    this.plot=document.getElementById("plot");this.canvas=document.getElementById("chartCanvas");this.ctx=this.canvas.getContext("2d");this.timeLabels=document.getElementById("timeLabels");this.candleTimer=document.getElementById("candleTimer");this.priceLine=document.getElementById("priceLine");this.priceScaleLabels=document.getElementById("priceScaleLabels");this.currentPriceEl=document.getElementById("currentPrice");
    this.candles=[];this.currentCandle=null;this.maxCandles=5000;this.currentPair='EUR/USD';this.pairStates=new Map();this._switching=false;
    const _cfg=PAIR_CONFIG['EUR/USD'];this.basePrice=_cfg.basePrice;this.seed=_cfg.seed;this.digits=_cfg.digits;this.vb=_cfg.vb;this.tb=_cfg.tb;
    this.currentPrice=this.basePrice;this.priceRange={min:this.basePrice*.99,max:this.basePrice*1.01};
    this.baseSpacing=12;this.zoom=1;this.targetZoom=1;this.minZoom=0.425;this.maxZoom=2.25;this.zoomEase=0.28;this.targetOffsetX=0;this.offsetX=0;this.panEase=0.38;this.velocity=0;this.drag=0;this.dragStartX=0;this.dragStartOffset=0;this.lastDragX=0;this.lastDragTime=0;this.pinch=0;this.p0=0;this.pMidX=0;this.pMidY=0;
  ...
  _createCandleControlPanel(){this._injectCCPStyles();if(document.getElementById('candleControlPanel'))return;const el=document.createElement('div');el.id='candleControlPanel';if(getComputedStyle(this.plot).position==='static')this.plot.style.position='relative';this.plot.appendChild(el);}
  showCandleControlPanel(){
    ...
    let timeLeft=10;panel.innerHTML=this._buildCCPHtml(timeLeft);panel.classList.add('ccp-visible');
    panel.querySelectorAll('.ccp-btn').forEach(btn=>{btn.addEventListener('click',()=>{const ts=parseInt(btn.closest('.ccp-row').getAttribute('data-ts')),dir=btn.getAttribute('data-dir');if(this._forcedCandles[ts]===dir){delete this._forcedCandles[ts];}else{this._forcedCandles[ts]=dir;const timeEl=btn.closest('.ccp-row').querySelector('.ccp-time'),timeStr=timeEl?timeEl.textContent:'';showInfoToast(dir==='up'?`🟢 شمعة ${timeStr} ستُغلق صاعدة ↗`:`🔴 شمعة ${timeStr} ستُغلق هابطة ↘`,dir==='up'?'buy':'sell',2200);}this._syncCCPSelections();});});
    const closeBtn=document.getElementById('ccpCloseBtn');if(closeBtn)closeBtn.addEventListener('click',()=>this.hideCandleControlPanel());
    this._syncCCPSelections();
    this._ccpCountdownTimer=setInterval(()=>{timeLeft--;const badge=document.getElementById('ccpTimerBadge'),fill=document.getElementById('ccpProgressFill');if(badge)badge.textContent=timeLeft+'s';if(fill)fill.style.width=(timeLeft*10)+'%';if(timeLeft<=0)this.hideCandleControlPanel();},1000);
  }
  ...
  async switchPair(newPair){
    ...
    this.currentPair=newPair;const cfg=PAIR_CONFIG[newPair]||PAIR_CONFIG['EUR/USD'];
  ...
  getIndexForCandleTimestamp(ts){if(ts===undefined||ts===null)return null;if(this.currentCandle&&this.currentCandle.timestamp===ts)return this.candles.length;for(let i=0;i<this.candles.length;i++)if(this.candles[i].timestamp===ts)return i;const tS=this.candles.length?this.candles[0].timestamp:this.t0;let idx=Math.round((ts-tS)/this.timeframe);idx=Math.max(0,Math.min(idx,this.candles.length));return idx;}
  clampPan(){const mn=this.getMinOffset(),mx=this.getMaxOffset();this.targetOffsetX=Math.max(mn,Math.min(mx,this.targetOffsetX));this.offsetX=Math.max(mn,Math.min(mx,this.offsetX));}
  snapToLive(){this.targetOffsetX=this.getMinOffset();this.offsetX=this.targetOffsetX;this.velocity=0;this.clampPan();}
  updatePan(){const diff=this.targetOffsetX-this.offsetX;if(Math.abs(diff)>.003)this.offsetX+=diff*this.panEase;else this.offsetX=this.targetOffsetX;if(Math.abs(this.velocity)>.01){this.targetOffsetX+=this.velocity;this.velocity*=.972;this.clampPan();}else this.velocity=0;}
  ...
  applyZoomAround(mx,my,sc){const oz=this.targetZoom,nz=Math.max(this.minZoom,Math.min(this.maxZoom,oz*sc));if(Math.abs(nz-oz)<1e-6)return;const idx=this.xToIndex(mx);this.targetZoom=nz;this.zoom=nz;const nx=mx-idx*this.getSpacing();this.targetOffsetX=nx;this.offsetX=nx;this.clampPan();this.updateTimeLabels();}
  indexToX(i){return this.offsetX+i*this.getSpacing();}
  xToIndex(x){return(x-this.offsetX)/this.getSpacing();}
  getPriceRange(){const mn=this.smin!==null?this.smin:this.priceRange.min,mx=this.smax!==null?this.smax:this.priceRange.max;return{min:mn,max:mx};}
  niceNum(v,rnd){const e=Math.floor(Math.log10(v)),p=Math.pow(10,e),f=v/p;let nf;if(rnd){if(f<1.5)nf=1;else if(f<3)nf=2;else if(f<7)nf=5;else nf=10;}else{if(f<=1)nf=1;else if(f<=2)nf=2;else if(f<=5)nf=5;else nf=10;}return nf*p;}
  calcNiceGrid(){const r=this.getPriceRange(),rng=r.max-r.min,d=this.niceNum(rng/7,0),g0=Math.floor(r.min/d)*d,g1=Math.ceil(r.max/d)*d;return{min:g0,max:g1,step:d,count:Math.round((g1-g0)/d)};}
  drawGrid(){
    const{min,max,step,count}=this.calcNiceGrid();
    for(let i=0;i<=count;i++){const p=min+i*step,y=this.priceToY(p);if(y<-5||y>this.h+5)continue;const mj=i%5===0;this.ctx.strokeStyle=mj?"rgba(255,215,0,.12)":"rgba(255,255,255,.05)";this.ctx.lineWidth=mj?1:.8;this.ctx.beginPath();this.ctx.moveTo(0,y+.5);this.ctx.lineTo(this.w,y+.5);this.ctx.stroke();}
    ...
    for(let i=s;i<=e;i++){if(i%stepC!==0)continue;const x=this.indexToX(i);if(x<-5||x>this.w+5)continue;const mj=i%Math.round(stepC*5)===0;this.ctx.strokeStyle=mj?"rgba(255,215,0,.12)":"rgba(255,255,255,.05)";this.ctx.lineWidth=mj?1:.8;this.ctx.beginPath();this.ctx.moveTo(x+.5,0);this.ctx.lineTo(x+.5,this.h);this.ctx.stroke();}
  }
  updateTimeLabels(){
  ...
  drawCandle(c,x,glow){
    ...
    this.ctx.fillRect(x-w/2,bt,w,bh);if(glow)this.ctx.shadowBlur=0;
  }
  addMarker(t,tradeData=null){
    const c=this.currentCandle;if(!c)return null;
    let fp;if(tradeData&&tradeData.markerPrice!==undefined){fp=tradeData.markerPrice;}else{const bt=Math.max(c.open,c.close),bb=Math.min(c.open,c.close),op=this.currentPrice;fp=op;if(op>bt)fp=bt;else if(op<bb)fp=bb;}
  ...
  updateCurrentCandle(){
    ...
    const n=Date.now(),r=this.rnd(this.seed+n),dir=(r-.5)*vb*0.5,t=this.currentCandle.close+dir,ms=vb*.18;
  ...
  initEvents(){
    ...
    this.canvas.addEventListener("touchstart",e=>{const r=this.canvas.getBoundingClientRect();if(e.touches.length===1)md(e.touches[0].clientX-r.left,Date.now());else if(e.touches.length===2){this.drag=0;this.pinch=1;this.p0=db2(e.touches[0],e.touches[1]);this.pMidX=(e.touches[0].clientX+e.touches[1].clientX)/2-r.left;this.pMidY=(e.touches[0].clientY+e.touches[1].clientY)/2-r.top;}},{passive:false});
    this.canvas.addEventListener("touchmove",e=>{e.preventDefault();const r=this.canvas.getBoundingClientRect();if(this.pinch&&e.touches.length===2){const d=db2(e.touches[0],e.touches[1]);if(this.p0>0){const sc=Math.max(.2,Math.min(5,d/(this.p0||d)));this.applyZoomAround(this.pMidX,this.pMidY,sc);}this.p0=d;}else if(!this.pinch&&e.touches.length===1){mm(e.touches[0].clientX-r.left,Date.now());}},{passive:false});
...
async function initChartSystem(){try{const isMaster=await window.masterManager.claimMaster(window.chart.currentPair);await window.chart.loadCandlesFromFirebase(null,isMaster);console.log(`✅ Chart ready | ${isMaster?'🎯 Master':'👁️ Viewer'} | ${window.chart.currentPair}`);}catch(e){console.error('initChartSystem error:',e);await window.chart.loadCandlesFromFirebase(null,true);}}
initChartSystem();
loadActiveTrades();

/* ══════════════════════════════════════════════════════
   ★★★  LONG-PRESS على BUY لمدة 10 ثوانٍ → لوحة التحكم
   ══════════════════════════════════════════════════════ */
(function(){
  ...
  function _startBar(){_createBar();const bar=document.getElementById('_lpBar');if(bar){bar.style.transition='none';bar.style.width='0%';requestAnimationFrame(()=>{bar.style.transition=`width ${HOLD_MS}ms linear`;bar.style.width='100%';});}}
  ...
  function _start(e){
    ...
    _startBar();
    _lpTimer=setTimeout(()=>{
      _lpTimer=null;_stopBar();
      if(window.chart)window.chart.showCandleControlPanel();
    },HOLD_MS);
  }
...
function openHistoryPanel(){document.getElementById('historyPanel').classList.add('show');document.getElementById('navHistory').classList.add('active');document.getElementById('navChart').classList.remove('active');loadHistory();}
function closeHistoryPanel(){document.getElementById('historyPanel').classList.remove('show');document.getElementById('navHistory').classList.remove('active');document.getElementById('navChart').classList.add('active');}
